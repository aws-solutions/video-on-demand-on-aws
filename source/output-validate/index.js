...
exports.handler = async (event) => {
  ...
  try {
    ...
    data.posterImageUrl = `https://${data.cloudFront}/${buildUrl(
      data.posterImage
    )}`;
    ...
    event.detail.outputGroupDetails.forEach((output) => {
      ...
      switch (output.type) {
        case 'HLS_GROUP':
          ...
          data.hlsUrl = `https://${data.cloudFront}/${buildUrl(
            data.hlsPlaylist
          )}`;
        ...
        case 'DASH_ISO_GROUP':
          ...
          data.dashUrl = `https://${data.cloudFront}/${buildUrl(
            data.dashPlaylist
          )}`;
        ...
        case 'FILE_GROUP':
          ...
          output.outputDetails.forEach((file) => {
            if (file.outputFilePaths) {
              ...
              urls.push(
                `https://${data.cloudFront}/${buildUrl(
                  file.outputFilePaths[0]
                )}`
              );
            }
          });
        ...
        case 'MS_SMOOTH_GROUP':
          ...
          data.mssUrl = `https://${data.cloudFront}/${buildUrl(
            data.mssPlaylist
          )}`;
        ...
        case 'CMAF_GROUP':
          ...
          data.cmafDashUrl = `https://${data.cloudFront}/${buildUrl(
            data.cmafDashPlaylist
          )}`;

          data.cmafHlsPlaylist = output.playlistFilePaths[1];
          data.cmafHlsUrl = `https://${data.cloudFront}/${buildUrl(
            data.cmafHlsPlaylist
          )}`;
