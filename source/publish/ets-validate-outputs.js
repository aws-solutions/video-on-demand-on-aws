...
exports.handler = (event, context, callback) => {
  ...
  if (event.preset === 'mp4') {
    bucket = process.env.Mp4Destination;
  } else {
     bucket = process.env.AbrDestination;
  }
  ...
  Promise.all(promises)
    .then(() => {
      ...
      if (event.preset === 'hls') {
        event[event.preset + 'Url'] = cfn + event.msg.playlists[0].name + '.m3u8';
      }

      if (event.preset === 'dash') {
        event[event.preset + 'Url'] = cfn + event.msg.playlists[0].name + '.mpd';
      }
