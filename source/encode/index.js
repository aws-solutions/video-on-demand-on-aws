...
exports.handler = async (event) => {
  ...
  try {
    ...
    var params = {
      Bucket: `${event.destBucket}/${event.guid}/poster`,
      CopySource: `/${event.srcBucket}/${event.posterImage}`,
    ...
    event.posterImage = `${outputPath}/poster/${event.posterImage}`;
    ...
    let tmpl = await mediaconvert
      .getJobTemplate({ Name: event.jobTemplate })
      .promise();
    ...
    tmpl.JobTemplate.Settings.OutputGroups.forEach((group) => {
      ...
      if (found) {
        ...
        // PR: https://github.com/awslabs/video-on-demand-on-aws/pull/107
        const outputGroup = mergeSettingsWithDefault(
