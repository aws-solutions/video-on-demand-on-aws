...
resource "opensearch_role" "λ_archive_source" {
  role_name           = "${local.project}-${local.archive_source_function_name}"
  description         = "Write access for ${local.project}-${local.archive_source_function_name} lambda"
  cluster_permissions = ["indices:data/write/bulk"]

  index_permissions {
    index_patterns  = ["${local.project}-${local.archive_source_function_name}-lambda-*"]
    allowed_actions = ["write", "create_index"]
  }
}

resource "opensearch_roles_mapping" "λ_archive_source" {
  role_name     = opensearch_role.λ_archive_source.role_name
  backend_roles = [module.λ_archive_source.role_arn]
}