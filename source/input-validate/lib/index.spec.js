...
describe('#INPUT VALIDATE::', () => {
  ...
  it('should succeed when processing valid metadata', async () => {
    const validMetadata = {
      Body:
        '{"srcVideo": "video_from_json.mp4", "archiveSource": false, "frameCapture": false, "srcBucket": "other-source", "jobTemplate_720p": "other-template"}',
    };
  ...
  it('should correctly handle metadata in PascalCase', async () => {
    const invalidMetadata = {
      Body:
        '{"srcVideo": "video_from_json.mp4", "ArchiveSource": false, "FrameCapture": false, "SrcBucket": "other-source", "inputRotate": "AUTO" }',
    };
  ...
  it('should fail when getting object from S3 throws an exception', async () => {
    ...
    await lambda
      .handler(_json)
      .catch((err) => expect(err).to.equal('S3 GET ERROR'));
  });
});
